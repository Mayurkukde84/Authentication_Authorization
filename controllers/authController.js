...
const login = asyncHandler(async(req,res)=>{
    ...
    if(!email || !password){
        return res.status(401).json({message:'Please filled login and password'})
    }
...
try {
    ...
    await user.save({validateBeforeSave:false})
    return next(res.status(500).json({message:'There was an error sending the email,Try again later!'}))
}
